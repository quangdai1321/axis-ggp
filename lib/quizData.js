...
// Dùng chung cho cả chủ đề có sẵn lẫn quiz người chơi tự tạo.
export function shuffleQuestions(questions) {
  return shuffle(questions).map((q) => {
    const order = shuffle(q.answers.map((_, i) => i));
    const answers = order.map((i) => q.answers[i]);
    const image = q.image || null; // ảnh minh hoạ (nếu có) đi kèm câu hỏi
    // câu nhiều đáp án đúng: `correct` là mảng chỉ số -> remap từng phần tử
    if (q.multi) {
      const correct = (Array.isArray(q.correct) ? q.correct : [q.correct])
        .map((c) => order.indexOf(c))
        .sort((a, b) => a - b);
      return { text: q.text, image, answers, multi: true, correct };
    }
    return { text: q.text, image, answers, correct: order.indexOf(q.correct) };
  });
}
...
export function buildQuizRun(topicId) {
  ...
  if (!topic) return [];
  return shuffleQuestions(topic.questions);
}
